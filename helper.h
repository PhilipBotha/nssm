...
namespace nssm {
    ...
    [[nodiscard]] std::vector<TCHAR> getVec(const TCHAR * apNullTermString);

    template<Char_c T>
    [[nodiscard]] std::vector<std::basic_string<T>> getArgs(int argc, const T *const argv[]){
        return std::vector<std::basic_string<T>>{argv, argv + argc};
    }
}
